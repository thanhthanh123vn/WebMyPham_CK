...
public class RemovePromotion extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        boolean isSuccess  =  dao.deletePromotion(promotion.getId());
        if(isSuccess){
            System.out.println("xóa mã giảm giá thành công");
        }else {
            System.out.println("xóa mã giảm giá không thành công");
        }
        ...
        resp.getWriter().write("{\"message\":\" xóa mã giảm giá thành công !\"}");
    }
